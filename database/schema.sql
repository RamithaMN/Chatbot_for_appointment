...
CREATE TABLE appointments (
    ...
    start_at TIMESTAMP WITH TIME ZONE GENERATED ALWAYS AS (
...
CREATE INDEX idx_appointments_tenant_start_at ON appointments(tenant_id, start_at);
...
-- Required index as specified in requirements
...
COMMENT ON COLUMN appointments.tenant_id IS 'Multi-tenant identifier for data isolation';
COMMENT ON COLUMN appointments.start_at IS 'Computed timestamp combining date and time';
...
BEGIN
    RAISE NOTICE '========================================';
    RAISE NOTICE 'Dental Chatbot Database Schema created successfully!';
    RAISE NOTICE 'Tables: users, appointments, chat_sessions, chat_messages, audit_log';
    RAISE NOTICE 'Required indexes: appointments(tenant_id, start_at), chat_messages(session_id, created_at)';
    RAISE NOTICE '========================================';
END $$;
